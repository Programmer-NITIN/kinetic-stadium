...
Design:
  ...
  - Peak hours boost density. Time-of-day shapes the base curve via a sine function.
  - Supports event-phase surge modifiers (halftime → amenities spike, exit → gates spike).
  - No external dependencies — pure Python math.
  - Results are cached for 2 seconds (see cache.py) so burst-repeated calls
    within the same polling cycle share one computation.
"""
...
def _base_density(
    ...
    time_factor = (math.sin((hour - 6) * math.pi / 12) + 1) / 2
    ...
    if event_phase == "halftime" and zone_type == "amenity":
        surge_boost = 35
    elif event_phase == "halftime" and zone_type == "restroom":
        surge_boost = 30
    elif event_phase == "exit" and zone_type == "gate":
        surge_boost = 45
    elif event_phase == "entry" and zone_type == "gate":
        surge_boost = 30
    elif event_phase == "live" and zone_type == "venue":
        surge_boost = 15

    # Zone-specific jitter for realistic variation
...
def get_zone_density_map(
    ...
    Results are cached at 2-second granularity.
    """
